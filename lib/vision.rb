...
module Vision
  class << self
    def get_image_data(image_key)
      # APIのURL作成
      api_url = "https://vision.googleapis.com/v1/images:annotate?key=#{ENV['GOOGLE_API']}"

      # 画像をbase64にエンコード
      #base64_image = Base64.encode64(open("#{Rails.root}/public/uploads/#{image_file.id}").read)
      base64_image = Base64.encode64(open(ActiveStorage::Blob.service.send(:path_for, image_key )).read)
